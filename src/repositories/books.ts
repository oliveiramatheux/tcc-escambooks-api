...
const getInfoBookByIsbn = async (isbn: string) => {
  const book = await axios.get(`${GOOGLE_API_BASE_URL}${GOOGLE_API_BASE_PATH}?q=isbn:${isbn}&key=${config.applicationKeyGoogleBooksApi}`)
  return book
}
...
const getBooksByUserId = async (userId: string) => {
  const book = await Book.find({ userId }) as IBookResponse[]
  return book
}
